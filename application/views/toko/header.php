	...
	<body style="transform-origin: 0 0;">
					...
					<div class="row">
						...
						<div class="col-md-3 clearfix">
							<div class="header-ctn">
								<!-- Wishlist -->
								<div>
									<a href="#modal_notif" id="1"  data-toggle="modal">
										<i class="fa fa-bell"></i>
										<span>Notifikasi Kredit</span>
										<?php $id=$_SESSION['customer_id'];?>
										<div class="qty"><?php $notif_kredit=$this->db->query("select count(*) as total from prospek where id_customer='$id' and status='DEAL'")->row_array()?><?=$notif_kredit['total']?></div>
									</a>
								</div>
								<!-- /Wishlist -->
		...
		<div class="modal fade" id="lihat_profil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog">
			...
			<!-- /MAIN HEADER -->
							<!-- container -->
		<div class="modal fade" id="modal_notif" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
			  <?php 
			  	$id=$_SESSION['customer_id'];
				 $data_kons=$this->db->query("SELECT * FROM prospek where id_customer='$id' and status='DEAL'")->result();
			  ?>
                <h4 class="modal-title" id="myModalLabel">&nbsp;Pengajuan Kredit di setujui</h4>
              </div>
              <div class="modal-body" id="IsiModal">
			 	<form action="<?php echo base_url('konsumen/update_action2')?>" method="post" enctype="multipart/form-data">
			  <table class="table">
			  	<thead>
				  <?php foreach($data_kons as $ds):?>
				  <tr>
				  <?php 
				 $kod=$ds->kode_barang; 
				  $foto=$this->db->query("select * from barang where kode_barang='$kod'")->row_array();?>
										<td><img src="<?= base_url('image/'.$foto['foto'])?>" alt="Foto Barang" width="100px" height="100px;"></td>
				  </tr>
					<tr>
					<td>Kode Barang</td>
					<td>:</td>
					<td><input type="text" name="nama_lengkap" id="nama_lengkap" class="form-control" value="<?=$ds->kode_barang?>"></td>
					</tr>  
					<tr>
					<td>Nama Barang</td>
					<td>:</td>
					<td><input type="text" name="nama_barang" id="no_ktp" class="form-control" value="<?=$ds->nama_barang
					?>"></td>
					</tr>  
					<tr>
					<td>Status</td>
					<td>:</td>
					<td><input type="text" name="no_hp" id="no_hp" class="form-control" value="<?=$ds->status?>"></td>
					</tr>  
					<tr>
					<td><a href="<?php echo base_url('toko/pengajuan_kredit')?>" class="btn btn-success btn-sm"><i class="fa fa-eye"></i> View</a></td>
					</tr>
				  <?php endforeach;?>
				</thead>
			  </table>
				...
				<div id="responsive-nav">
					...
					<ul class="main-nav nav navbar-nav">
						...
						<!-- <li><a href="<?php echo base_url('toko/kontak_kami')?>">Kontak Kami</a></li> -->
							...
							$not=$this->db->query("SELECT count(*) as notif from prospek where id_customer='$id_cus' and status='baru'")->row_array()?>
						...
						<li><a href="<?php echo base_url('toko/tracking')?>">Tracking</a></li>
					</ul>
					<!-- /NAV -->
				</div>
				<!-- /responsive-nav -->
